...
The test suite contains **21 automated unit and integration tests** verifying:
1. **Series Resistors**: $R_{\text{series}} = R_1 + R_2$.
2. **Parallel Resistors**: $R_{\text{parallel}} = (R_1 R_2) / (R_1 + R_2)$.
3. **Triangle Network**: $1\,\Omega \parallel 2\,\Omega = 2/3\,\Omega$.
4. **Wheatstone Bridge**: Exact Delta-Wye analytical transformation.
5. **Joule Heating Energy Conservation**: $\sum I_{ij}^2 R_{ij} = R_{\text{eff}} \cdot I^2$.
6. **Parallel RC Impedance Circuit**: Analytical cutoff frequency $f_c = \frac{1}{2\pi R C}$.
7. **Percolation Disconnection**: Returns $R_{\text{eff}} = \infty$ for disconnected components.
8. **Statistical Distributions**: Log-normal mean and variance moment convergence.
...
1. **Kirchhoff's Matrix Tree Theorem & Resistance Distance**:
   - Klein, D. J., & Randić, M. (1993). *Resistance distance*. Journal of Mathematical Chemistry, 12(1), 81-95.
2. **Transport in 2D Material Composites & Nanosheet Networks**:
   - O'Callaghan, C., et al. (2016). *Effective resistance and scaling in disordered nanosheet networks*. Physical Review B.
   - Coleman, J. N., et al. (2011). *Two-dimensional nanosheets produced by liquid exfoliation of layered materials*. Science, 331(6017), 568-571.
3. **Percolation Theory in Disordered Conductors**:
...
4. **Electrical Impedance Spectroscopy**:
   - Barsoukov, E., & Macdonald, J. R. (2018). *Impedance Spectroscopy: Theory, Experiment, and Applications*. John Wiley & Sons.
